    ...
    <div className="flex flex-col items-start gap-1.5">
      <div className="inline-flex items-center gap-2">
        <button
          ...
          aria-label={
            isPending
              ? "Digər cihazlar çıxarılır"
              : armed
                ? "Digər cihazlardan çıxışı təsdiqləyin"
          ...
          className={`inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-xs font-semibold transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-60 ${
            armed
              ...
              : "border border-rose-200/90 bg-white text-rose-600 hover:border-rose-300 hover:bg-rose-50/80"
          }`}
        >
          {isPending
            ? "Çıxarılır..."
        ...
        {armed && !isPending && (
          <button
            ...
            className="rounded-lg bg-slate-100 px-2.5 py-2 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-200 hover:text-slate-900"
          >
            İmtina
          </button>
        )}
      ...
      {result?.error && (
        <p role="alert" className="animate-fadeIn text-[11px] font-medium text-rose-600">
          {result.error}
        </p>
      )}
      {result?.success && (
        <p role="status" className="animate-fadeIn text-[11px] font-medium text-emerald-600">
          Digər bütün cihazlardakı aktiv sessiyalar bağlandı.
        </p>
      )}
    </div>
  );
